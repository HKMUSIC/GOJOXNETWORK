...
async def search_command(_, message):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(
            ...
            prev_and_next_btns = [
                [InlineKeyboardButton("▶Next▶", callback_data=f"next {start+10} {message.text.split()[1]}")]
            ]
...
async def prev_callback(_, callback_query):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            ...
            prev_and_next_btns = [
                [InlineKeyboardButton("◀Prev◀", callback_data=f"prev {start-10} {(callback_query.data.split()[2]).decode('utf-8')}"),
                 InlineKeyboardButton("▶Next▶", callback_data=f"next {start+10} {(callback_query.data.split()[2]).decode('utf-8')}")]
            ]
